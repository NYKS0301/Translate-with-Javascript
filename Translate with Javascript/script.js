...
btnTranslate.addEventListener("click", () => {
    ...
    let to = toLang.value;
    const url = `https://api.mymemory.translated.net/get?q=${text}&langpair=${from}|${to}`; // https://mymemory.translated.net/doc/spec.php //MyMemory
    ...
    icon.addEventListener("click", (element) => {
        if(element.target.classList.contains("fa-copy")) {
           if(element.target.id == "from") {
            ...
            let utterance;
            if(element.target.id == "from") {
                if (fromText.value == "Selam") {
                    utterance = new SpeechSynthesisUtterance(fromText.value + "Patron");
...
// https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesisUtterance //mdn web docs_